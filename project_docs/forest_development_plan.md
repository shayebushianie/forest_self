...
> **安装约定**：所有工具链和依赖库（Qt SDK、CMake、编译器）**必须安装到本项目目录 `D:\No.6CollaborationProject\forest\` 下或其子目录中**，禁止使用系统全局路径，确保项目可移植、可打包。
...
| 5.7 | **便携打包（windeployqt）**：编译 Release 版本 → 用本地 `Qt/6.5.3/msvc2019_64/bin/windeployqt.exe` 自动抓取依赖 DLL 到 exe 同级目录 → 压缩 `Release` 文件夹为绿色便携包（~50MB），老师解压即可双击运行 | 交付目录 `artifacts/forest_portable.zip` | 2h | 5.6 |
| 5.8 | 源码打包：仅提交 `src/`、`CMakeLists.txt`、`project_docs/`、`scripts/`、`.gitignore`，**严格排除 `Qt/` 和 `build/` 目录** | 交付目录 `artifacts/forest_source_code.zip` | 1h | 5.7 |
...
**问题描述**：代码中写死 `"D:/No.6CollaborationProject/forest/sessions.dat"` → 项目拷贝到其他电脑（如老师 `C:/Users/Downloads`）→ 路径不存在或无写入权限 → 程序崩溃。
...
# 将 Qt 安装到 D:\No.6CollaborationProject\forest\Qt\ 下
...
|:----:|:--------:|------|:------:|----------|
| **第 0 步** | — | **安装 Qt SDK 到本目录**：下载 Qt 6.5.x（MSVC 2019 64-bit），安装路径选 `D:\No.6CollaborationProject\forest\Qt\`；安装 CMake（≥3.16）；确认 MSVC 编译器可用（Visual Studio 2019/2022 Community 均可，MSVC 允许全局安装，仅 Qt SDK 和 CMake 约束在项目本地） | 2h | `Qt/6.5.3/msvc2019_64/bin/qmake.exe` 存在 |
...
1. **所有依赖安装到本目录，绝不散落系统各处**：Qt SDK → `forest\Qt\`；CMake 构建产物 → `forest\build\`；数据文件（`sessions.dat`、`coins.dat`） → 可执行文件同目录。**一切路径相对于项目根目录**，确保拷贝到其他电脑或老师批改时直接可用，无需重新配置。
...
| 21 | **便携打包（windeployqt）** | 绿色便携包 | ✅ `artifacts/forest_portable.zip`（23 MB） |
...
├── .gitignore                     (18 行)
├── CLAUDE.md
├── project_docs/forest_development_plan.md      (含 §七~§十一 完成状态)
├── docs/
│   ├── proposal.md                (~1800 字)
...
打包（禁止抓取系统 MinGW DLL，避免 UCRT/MSVCRT 冲突）：
  ...
  Compress-Archive -Path build\* -DestinationPath artifacts/forest_portable.zip

源码打包：
  Compress-Archive -Path src, docs, CMakeLists.txt, .gitignore ^
    -DestinationPath artifacts/forest_source_code.zip
```
...
| 修复后 | 启动成功，进程持续运行 | ✅ | `artifacts/forest_portable.zip` 23 MB，解压即可双击运行 |

### 11.6 沙箱测试清单（打包后必做）

> 将 `artifacts/forest_portable.zip` 拷贝到一台**没有安装 Qt、没有 VS 环境**的干净电脑上解压运行：
...
| `artifacts/forest_portable.zip` | 23.4 MB | 绿色便携包 |
| `artifacts/forest_source_code.zip` | ~40 KB | 源码包 |
...
├── .gitignore
├── CLAUDE.md
├── project_docs/forest_development_plan.md
├── artifacts/forest_portable.zip       (23.7 MB)
├── docs/
│   ├── proposal.md
│   ├── test_report.md
│   └── final_report.md
...
## 十八、工程优化记录（2026-06-10）

### 18.1 优化目标

本次优化不追加新功能，重点修正“计划文档承诺”和“工程实际行为”之间的偏差，保持改动小、可验证、便于答辩说明。

| 目标 | 验证方式 |
|------|----------|
| 标签统计真实可用 | 设置页选择标签后，`FocusRecord.tagId` 写入本次专注记录 |
| 金币结算只发生一次 | 成功状态多次触发 UI 刷新时，不重复调用 `CoinManager::earn()` |
| 完成/失败后可继续下一轮专注 | `SUCCESS/FAILED` 状态允许启动新会话 |
| 关闭深度专注时金币与记录一致 | 金币按“纯粹专注时间 × 梯度系数”结算 |
| 钱包数据语义清晰 | 当前余额与历史累计金币分开保存，避免消费后影响累计成就 |
| 版本库更轻 | `.gitignore` 补充 `artifacts/forest_portable/`、日志和压缩包规则 |

### 18.2 代码变更摘要

| 文件 | 优化点 |
|------|--------|
| `src/core/FocusController.h/.cpp` | `startFocus()` 增加 `tagId` 参数；成功/失败后允许重新开始；记录 `lastCoinsEarned()`；关闭深度专注时按纯粹专注时间计算金币 |
| `src/ui/MainWindow.h/.cpp` | 将设置页/预设的标签传入控制器；增加 `terminalStateHandled_` 防止成功状态重复结算金币、重复弹窗 |
| `src/core/CoinManager.h/.cpp` | `WalletRecord` 拆分 `balance` 与 `totalEarnedCoins`，兼容旧钱包文件并保留成就位图 |
| `src/storage/DatabaseManager.cpp` | `updateById()` 写回时强制修正 `recordId`，避免调用方传入旧记录导致主键漂移 |
| `.gitignore` | 增补打包目录、日志、安装日志和压缩包忽略规则，减少仓库噪音 |

### 18.3 后续建议

| 优先级 | 建议 | 原因 |
|:--:|------|------|
| P1 | 将已被 Git 跟踪的 `artifacts/forest_portable/` 中间构建文件从版本库移除，仅保留最终 zip 或重新生成包 | 当前仓库记录了大量可再生文件，会拖慢提交、比对和分发 |
| P1 | 增加最小自动化测试：`FocusController` 成功结算、标签写入、钱包消费后累计金币不下降 | 这些是核心数据正确性，手测容易漏 |
| P2 | 把全局 QSS 从 `main.cpp` 拆到 `src/resources.qrc` 或独立 `.qss` | 降低入口文件长度，便于后续视觉维护 |
| P2 | 为二进制记录增加 `magic/version` 字段的新格式迁移方案 | 现有 64 字节格式可用，但长期演进需要版本识别 |

---

## 十九、视觉与交互细节优化计划（2026-06-17）

### 19.1 优化目标

本轮优化不新增业务功能，重点提升现有 Qt Widgets 桌面端的主题质感、交互反馈和主画面丰富度。改动保持外科式范围，优先复用现有 `QSS`、`QPainter` 和页面结构，避免引入新的三方依赖或重写信息架构。

| 目标 | 验证方式 |
|------|----------|
| 主题配色更丰富，不再只有单一深绿层次 | 全局 QSS 中形成背景、卡片、主按钮、辅助色、警告色、焦点色的明确色阶 |
| 主要按钮和导航有更清晰的 hover/pressed/focus/disabled 状态 | 手动运行程序，逐页检查按钮、侧边栏、输入框和列表反馈 |
| 专注首页更流畅、更有沉浸感 | `PlantTimerWidget` 背景、圆环、旋钮、中心植物绘制层次增强；拖动旋钮无明显跳变 |
| 森林画布更像可浏览的成果区域 | `GardenCanvas` 增加渐变背景、网格层次、地块高光/阴影和成功/枯萎状态差异 |
| 商城、设置、历史等页面视觉统一 | 卡片、分组框、滚动条、列表项样式与主主题一致 |
| 不破坏现有专注、金币、记录、托盘等业务链路 | `cmake --build build --config Release` 编译通过，并完成一次基础手测 |

### 19.2 改动范围

| 模块 | 计划改动 |
|------|----------|
| `src/main.cpp` | 调整全局 `app.setStyleSheet(...)`：补充滚动条、焦点态、pressed 态、disabled 态、输入框 hover/focus、菜单圆角等细节 |
| `src/ui/MainWindow.cpp` | 优化侧边栏配色、导航 active 状态、页面背景渐变、预设卡片、商城卡片和常用按钮光标反馈 |
| `src/ui/PlantTimerWidget.cpp` | 增强计时圆环绘制：背景柔光、双层轨道、进度弧颜色层次、旋钮高光、中心植物阴影和文字层次 |
| `src/ui/GardenCanvas.cpp` | 增强森林画布：渐变地表、轻量纹理网格、地块阴影/描边、成功与失败地块色彩差异 |
| `src/ui/HistoryWidget.cpp` / `TimelineItemWidget.cpp` | 视情况微调时间线颜色、节点高光和列表 hover，不改变记录数据逻辑 |
| `docs/test_report.md` | 如完成实现，补充视觉手测记录：窗口打开、页面切换、旋钮拖动、滚动、按钮状态 |

### 19.3 WBS 拆分

| 编号 | 任务 | 产出 | 预计工时 | 验证 |
|------|------|------|:--:|------|
| 19.1 | 梳理现有 QSS 与自绘控件颜色 | 当前颜色表和低风险改动点 | 0.5h | 确认仅涉及视觉层 |
| 19.2 | 优化全局控件主题 | `main.cpp` 全局 QSS 更新 | 1h | 登录框、设置页、菜单和列表显示正常 |
| 19.3 | 优化主窗口页面与导航 | `MainWindow.cpp` 页面背景、侧栏、卡片、按钮反馈更新 | 1.5h | 四个页面切换正常，侧栏收起/展开动画正常 |
| 19.4 | 优化计时圆环和中心植物 | `PlantTimerWidget.cpp` 自绘效果增强 | 2h | 旋钮可拖动，倒计时/正计时显示正常 |
| 19.5 | 优化森林画布 | `GardenCanvas.cpp` 背景、网格、地块绘制增强 | 1.5h | 成功/失败记录按 `gridIndex` 正确显示 |
| 19.6 | 构建与手测 | 编译通过，记录基础手测结果 | 1h | `cmake --build build --config Release` 成功 |
...
|---|--------|:--:|
| 1 | 程序可编译，启动不崩溃 | 待验证 |
| 2 | 登录框、主窗口、弹窗主题色统一 | 待验证 |
| 3 | 侧边栏 active/hover/收起状态清晰 | 待验证 |
| 4 | 首页预设卡片和主操作按钮有 hover/pressed 反馈 | 待验证 |
| 5 | 计时圆环拖动和运行显示正常 | 待验证 |
| 6 | 森林画布背景、网格、成功/失败地块有层次 | 待验证 |
| 7 | 设置页输入框、下拉框、列表、滚动条观感统一 | 待验证 |
| 8 | 未修改专注计时、金币结算、记录写入等业务逻辑 | 待验证 |

### 19.5 风险与约束

| 风险 | 处理方式 |
|------|----------|
| 当前部分中文字符串存在编码显示异常，视觉优化可能掩盖不了文案乱码 | 本轮不主动大范围修复文案编码；另立任务处理，避免视觉改动扩大 |
| 全局 QSS 可能影响所有弹窗和列表 | 改动后逐页手测，发现局部冲突再用 objectName 精准覆盖 |
| 自绘控件过度复杂会增加维护成本 | 只使用 `QPainter` 基础渐变、阴影和描边，不引入新资源或三方库 |
| 已有打包产物和构建目录较多，容易混入无关差异 | 只提交 `src/` 和文档必要改动，不触碰 `build/`、`artifacts/forest_portable/`、zip 包 |

---

## 二十、主页计时模式与全局交互动画优化记录（2026-06-17）

### 20.1 优化目标

本轮在“桌面薄荷 Forest”视觉主题基础上，补齐主页计时模式入口和全局交互动画，让电脑端界面既保留桌面信息密度，又具备接近手机端参考图的轻盈切换体验。

| 目标 | 验证方式 |
|------|----------|
| 主页直接提供“倒计时 / 正计时”切换 | 首页分段按钮可切换模式，并同步设置页计时模式 |
| 倒计时提供暂停许可选项 | 取消“允许暂停”后，专注运行中暂停按钮不可用 |
| 倒计时提供深度专注开关 | “深度专注”开启时违规进入警告失败链路；关闭时仅折算收益 |
| 倒计时结束可选择自动延长 | 开启后倒计时归零不立即结算，自动转入正计时延长 |
| 所有常用控件有流畅交互反馈 | 按钮、复选框、下拉框、输入框具备 hover / press / focus 淡入淡出动画 |
| 模式选项切换不突兀 | 正计时时倒计时选项区折叠隐藏，倒计时时平滑展开 |

### 20.2 代码变更摘要

| 文件 | 变更 |
|------|------|
| `src/core/FocusController.h/.cpp` | `startFocus()` 增加 `allowPause` 和 `autoExtendCountdown` 参数；倒计时结束可自动转入正计时；暂停逻辑受 `allowPause_` 控制 |
| `src/ui/MainWindow.h/.cpp` | 首页新增“倒计时 / 正计时”分段切换、允许暂停、深度专注、结束后自动延长选项；新增主页模式与设置页模式双向同步 |
| `src/ui/PlantTimerWidget.cpp` | 正计时空闲态显示 `00:00`，避免继续显示倒计时分钟数 |
| `src/main.cpp` | 新增 `SmoothInteractionFilter` 全局事件过滤器，为按钮、复选框、下拉框、输入框添加透明度缓动动画 |
| `src/ui/MainWindow.cpp` | 为倒计时选项区添加高度展开 / 折叠动画，切换正计时和倒计时时不再跳变 |
...
|---|--------|:--:|
| 1 | 首页可切换倒计时 / 正计时 | 已完成 |
| 2 | 正计时隐藏倒计时专属选项，并显示 `00:00` | 已完成 |
| 3 | 倒计时可配置是否允许暂停 | 已完成 |
| 4 | 倒计时可配置深度专注开关 | 已完成 |
| 5 | 倒计时结束可自动延长为正计时 | 已完成 |
| 6 | 控件 hover / press / focus 有动画反馈 | 已完成 |
| 7 | 倒计时选项区展开 / 收起有动画 | 已完成 |
| 8 | Release 构建通过 | 已完成：`cmake --build build --config Release` |

### 20.4 后续建议

| 优先级 | 建议 | 原因 |
|:--:|------|------|
| P1 | 增加一次手动 UI 回归测试记录到 `docs/test_report.md` | 本轮涉及主页入口、运行态按钮禁用、倒计时结束自动延长等交互路径 |
| P1 | 修复现有中文字符串编码异常 | 当前部分 `QStringLiteral` 文案在源码和界面中存在乱码风险，影响最终观感 |
| P2 | 将全局 QSS 和交互动画类拆出 `main.cpp` | 入口文件已承担较多视觉逻辑，后续维护可拆到独立样式/辅助模块 |

---

## 二十一、样式拆分、编码核查与 UI 回归补充记录（2026-06-17）

### 21.1 优化目标

本轮针对上一阶段遗留建议进行收尾：将入口文件中的视觉样式与交互动画逻辑拆出，核查主要 UI 中文文案编码，并补充 UI 回归测试记录，降低后续维护成本。

| 目标 | 验证方式 |
|------|----------|
| `main.cpp` 不再承载大段 QSS 和动画过滤器实现 | 入口文件仅调用 `AppStyle::installSmoothInteractions(app)` 和 `AppStyle::styleSheet()` |
| 全局 QSS 与控件动画集中维护 | 新增 `src/ui/AppStyle.h/.cpp`，并加入 `CMakeLists.txt` |
| 主要 UI 中文文案为正常 UTF-8 | 使用 `rg` 检查登录页、主页、设置页、弹窗、测试报告新增章节等中文内容 |
| UI 回归测试有可追溯记录 | `docs/test_report.md` 新增“UI 回归测试记录（2026-06-17）” |
| 构建链路保持可用 | `cmake --build build --config Release` 成功 |

### 21.2 代码与文档变更

| 文件 | 变更 |
|------|------|
| `src/ui/AppStyle.h` | 新增样式辅助类声明，暴露 `styleSheet()` 与 `installSmoothInteractions()` |
| `src/ui/AppStyle.cpp` | 承载全局 QSS 与 `SmoothInteractionFilter`，集中管理 hover / press / focus 动画 |
| `src/main.cpp` | 移除大段 QSS 与事件过滤器实现，入口文件只保留样式安装调用 |
| `CMakeLists.txt` | 将 `src/ui/AppStyle.cpp` 加入构建目标 |
| `docs/test_report.md` | 新增 15 项 UI 回归测试记录，覆盖主页模式、动画、页面切换、金币反馈等 |

### 21.3 验收清单

| # | 检查项 | 状态 |
|---|--------|:--:|
| 1 | `AppStyle.cpp` 参与 Release 构建 | 已完成 |
| 2 | `main.cpp` 样式逻辑缩减为调用辅助类 | 已完成 |
| 3 | 主要 UI 中文文案通过 `rg` 验证为正常 UTF-8 | 已完成 |
| 4 | `docs/test_report.md` 补充 UI 回归测试章节 | 已完成 |
| 5 | `cmake --build build --config Release` 通过 | 已完成 |

### 21.4 后续建议

| 优先级 | 建议 | 原因 |
|:--:|------|------|
| P1 | 后续如继续修复历史文档乱码，优先处理 `project_docs/forest_development_plan.md` 和旧测试报告主体 | 历史文档在部分终端中仍会显示 mojibake，但新增章节和源码内容已为正常 UTF-8 |
| P2 | 为关键 UI 路径增加自动化测试或录屏验收 | 当前 UI 回归记录为手动测试表，自动化可降低后续主题迭代风险 |

---

## 二十二、登录页按钮可读性与便携包复验记录（2026-06-17）

### 22.1 本轮目标

用户反馈登录界面“注册”按钮颜色与背景过于接近，浅色对话框上按钮文字不够清晰。本轮以最小改动修复登录页按钮对比度，并重新生成 `forest_portable`，确认打包产物没有启动入口点错误。

| 目标 | 验证方式 |
|------|----------|
| “注册”按钮在登录页背景上清晰可见 | 为注册按钮设置 `btnSecondary`，使用深绿色按钮底色与白色文字 |
| 不影响主按钮与其它页面按钮 | 保留 `btnPrimary` 登录按钮样式，仅新增 secondary 覆盖样式 |
| Release 构建可用 | 执行 `cmake --build build --config Release` |
| 便携包运行时 DLL 匹配当前编译器 | 从当前 g++ 所在 `mingw64/bin` 复制 `libgcc_s_seh-1.dll`、`libstdc++-6.dll`、`libwinpthread-1.dll` |
| 避免 `clock_gettime64` 入口点错误 | 校验 `libwinpthread-1.dll` 导出/包含 `clock_gettime64` 后再压缩 |

### 22.2 代码与打包变更

| 文件/产物 | 变更 |
|------|------|
| `src/ui/LoginDialog.cpp` | 为“注册”按钮新增 `btnSecondary` 对象名 |
| `src/ui/AppStyle.cpp` | 新增 `QPushButton#btnSecondary` 的正常、hover、pressed 状态配色 |
| `artifacts/forest_portable/` | 使用最新 Release exe 重新生成绿色便携目录 |
| `artifacts/forest_portable.zip` | 使用最新 `artifacts/forest_portable/` 重新压缩 |

### 22.3 验收清单

| # | 检查项 | 状态 |
|---|--------|:--:|
| 1 | 登录页“注册”按钮对比度提升 | 已完成 |
| 2 | Release 构建通过 | 已通过：`cmake --build build --config Release` |
| 3 | `forest_portable` 重新生成 | 已通过 |
| 4 | `libwinpthread-1.dll` 包含 `clock_gettime64` | 已通过：`rg -a clock_gettime64 artifacts/forest_portable/libwinpthread-1.dll` |
| 5 | 便携版 `forest.exe` 启动检查无入口点错误 | 已通过：启动后持续运行 3 秒，随后结束进程 |

---

## 二十三、成品功能同步、视觉可读性与商城交互补强记录（2026-06-18）

### 23.1 本轮目标

本轮以当前源码工程为基础，吸收 `focus_forest_portable_previous` 成品目录中更适合交付演示的优点，同时保留当前工程已有的多账号、成就系统、双计时模式、深度专注开关、抽屉式导航和自动化便携打包能力。重复功能不新增第二套实现，只调整入口和交互，使功能边界更清晰。

| 目标 | 验证方式 |
|------|----------|
| 成品目录不再污染源码状态 | `.gitignore` 忽略 `artifacts/focus_forest_portable_previous/` |
| 运行数据集中在 `app_data/` | `users.dat`、日志和用户沙箱均通过 `PathConfig` 解析到 exe 同级 `app_data` |
| 兼容旧成品数据布局 | 启动时可从 exe 同级 `users.dat` 或 `app_data/app_data/users.dat` 迁移到 `app_data/users.dat` |
| 数据统计与时间历程成为独立入口 | 左侧导航新增“数据统计”“时间历程”，不再把历史列表和森林画布挤在同一页 |
| 登录和弹窗可读性提升 | 浅色弹窗内标签、输入框、按钮使用深色文字和高对比按钮配色 |
| 植物商城更可浏览 | 每个植物选项预留固定图片槽，鼠标悬停时卡片背景和边框高亮 |
| 便携包生成可复现 | 新增脚本完成构建、`windeployqt`、运行库复制、`clock_gettime64` 校验和启动烟测 |

### 23.2 代码与脚本变更

| 文件/模块 | 变更 |
|------|------|
| `.gitignore` | 新增 `artifacts/focus_forest_portable_previous/` 忽略规则，避免 DLL 和成品 exe 混入源码变更 |
| `src/common/PathConfig.h` | 新增 `getAppDataDir()`、`getAppDataFilePath()`、`migrateLegacyFileToAppData()`，统一管理便携运行数据路径 |
| `src/main.cpp` | 日志写入 `app_data/app.log`；用户库迁移并读取 `app_data/users.dat` |
| `scripts/package_portable.ps1` | 新增一键便携打包脚本，优先从 CMake 当前编译器目录复制 MinGW 运行库，并校验 `libwinpthread-1.dll` |
| `src/ui/MainWindow.h/.cpp` | 新增“数据统计”“时间历程”导航页、侧栏金币显示、统计摘要卡片；商城卡片增加植物图片槽和 hover 高亮 |
| `src/ui/AppStyle.cpp` | 为 `QDialog` 内标签、输入框、列表和按钮增加浅色弹窗专用高对比样式 |
| `src/ui/LoginDialog.cpp` | 登录标题、用户名/密码标签、状态提示改为更深的绿色文字 |
| `src/ui/StatisticsDialog.cpp` / `src/ui/StoreDialog.cpp` | 修复浅色弹窗上浅色文字的问题；商城弹窗同步植物图片槽和卡片悬停高亮 |

### 23.3 验收清单

| # | 检查项 | 状态 |
|---|--------|:--:|
| 1 | `artifacts/focus_forest_portable_previous/` 不再作为未跟踪成品目录参与源码提交 | 已完成 |
| 2 | 登录页标题、标签、输入框占位文字、登录/注册按钮对比度清晰 | 已完成：已启动 `build/forest.exe` 截图核对 |
| 3 | 统计弹窗、商城弹窗不再出现浅色背景上的浅色文字 | 已完成 |
| 4 | 左侧导航包含“专注主页、我的森林、植物商城、数据统计、时间历程、系统设置” | 已完成 |
| 5 | “我的森林”只保留森林画布，“时间历程”独立承载历史列表 | 已完成 |
| 6 | “数据统计”页展示专注次数、累计分钟、成功率、累计金币，并可打开标签时间分布图 | 已完成 |
| 7 | 主界面商城每个植物选项有固定图片槽，鼠标悬停卡片高亮 | 已完成 |
| 8 | 独立商城弹窗同样有固定图片槽和卡片 hover 高亮 | 已完成 |
| 9 | Release 构建通过 | 已通过：`cmake --build build --config Release` |
| 10 | 便携打包与启动烟测通过 | 已通过：`scripts/package_portable.ps1 -NoBuild -OutputDir "build\portable_verify" -SmokeTest` |

### 23.4 后续建议

| 优先级 | 建议 | 原因 |
|:--:|------|------|
| P1 | 为银杏树、向日葵、仙人掌补充独立图片资源 | 当前先复用已有 `oak/rose/pine` 图片占位，UI 空间已预留，后续只需替换资源路径 |
| P1 | 补充本轮 UI 回归记录到 `docs/test_report.md` | 本轮涉及导航入口、弹窗可读性、商城 hover 交互，测试报告应保持同步 |
| P2 | 若继续保留 `StoreDialog`，考虑让主界面商城和弹窗商城复用同一份植物商品数据 | 避免后续新增植物时两处列表不一致 |
