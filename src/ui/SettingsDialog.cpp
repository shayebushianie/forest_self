    ...
    : QDialog(parent), ruleEngine_(ruleEngine)
{
    ...
    plantCombo_->addItem(QStringLiteral("橡树 (OakTree)"), 0);
    plantCombo_->addItem(QStringLiteral("松树 (PineTree)"), 1);
    plantCombo_->addItem(QStringLiteral("玫瑰 (Rose)"), 2);
    ...
    focusModeCombo_ = new QComboBox;
    focusModeCombo_->addItem(QStringLiteral("严格模式 (切到游戏立即枯萎)"), 0);
    focusModeCombo_->addItem(QStringLiteral("温和模式 (切屏不枯，金币减半)"), 1);
    focusLayout->addWidget(focusModeCombo_);
...
bool SettingsDialog::isGentleMode() const
{
    return focusModeCombo_->currentData().toUInt() == 1;
}
